...
An encryption key is relatively important, and it's why we generate one instead of committing it to the Git repository - where it is vulnerable to attacks. To generate a key, simply run the command:
...
### 1.10 Migrate and Seed Database
...
To check if the installation went successfully, go to localhost in your browser. If successful, you should see a landing page.
...
```sh
docker-compose run --rm artisan make:model ModelName -a
```
...
Controller
```sh
docker-compose run --rm artisan make:controller ControllerName
```

Seeder
```sh
docker-compose run --rm artisan make:seeder SeederName
```

Migration
```sh
docker-compose run --rm artisan make:migration MigrationName
```

Factory
```sh
docker-compose run --rm artisan make:factory FactoryName
```
...
```sh
docker-compose run --rm artisan make:test ExampleTest
```
...
```sh
docker-compose run --rm artisan make:test ExampleTest --unit
```
